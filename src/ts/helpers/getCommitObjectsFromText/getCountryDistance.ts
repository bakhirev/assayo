...
export function getVpnList(countries: any[]) {
  ...
  for (let i = 0; i < countries.length; i++) {
    ...
    if (!to || !next) continue;
    const isFast = (next?.from - to?.from) < ONE_DAY;
    const isThisPlace = from.fromTimezone === next.fromTimezone;
    if (isFast && isThisPlace) {
      vpn.set(to.title, to.fromTimestamp);
      i += 1;
    }
  }
...
export function getTravels(countries: any[], vpnList: HashMap<string>) {
  ...
  const formattedCountries = countries.length > 3
    ? countries.filter((item: any) => !vpnList.has(item.title))
    : countries;
