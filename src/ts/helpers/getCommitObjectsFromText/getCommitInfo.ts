...
export default function getCommitInfo(logString: string): ICommit | ISystemCommit {
  ...
  if (isSystemCommit) {
    ...
    let prId, prExternalId, repository, branch, toBranch, task, taskNumber, type, scope, server;
    if (isGithubPR) {
      ...
      [prExternalId, repository, branch, toBranch] = getGithubPrInfo(message);
      prId = uniqPrId++;
      task = getTask(branch);
      ...
      const firstIndex = 14;
      const match = message.substring(firstIndex).match(/[^0-9]/) || { index: 0 };
      const lastIndex = firstIndex + (match.index || 0);
      prExternalId = message.substring(firstIndex, lastIndex);
      prId = uniqPrId++;
      const description = message.substring(lastIndex + 1).trim();
      commonInfo.message = description;
      ...
      if (branchToParse) {
        ...
        prId = taskNumber ? `#${taskNumber}-${Math.random()}` : '';
