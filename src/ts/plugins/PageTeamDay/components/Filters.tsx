...
function getFormattedUsers(rows: any[], text: Function) {
  const options = rows.map((title: string, id: number) => ({ id: id + 1, title }));
  options.unshift({ id: 0, title: text('page.team.tree.filters.all') });
  return options;
}
...
const TempoFilters = observer(({
  ...
  const users = useMemo(() => getFormattedUsers(authors, text), [authors]);
