...
function getTimeZoneChart(authors: any[]) {
  const details = authors.reduce((acc: any, author) => {
    increment(acc, author.lastCommitTimezone.replace(':', '.'));
    return acc;
  }, {});
...
function PieCharts({ authors, countries }: PieChartsProps): React.ReactElement | null {
  ...
  return (
    <Section>
      <SectionColumn>
        <PieChart
          title="plugin.team_country.pieByDomain.title"
          suffix="plugin.team_country.chart.item"
          order={countryOrder}
          limit={countryLimit}
      ...
      <SectionColumn>
        <PieChart
          title="plugin.team_country.pieByTimezone.title"
          suffix="plugin.team_country.chart.item"
          order={timezoneOrder}
          limit={3}
